...
if (isset($_POST['checkin']) && isset($_POST['checkout'])) {
   // $customer_route = $_POST['customer_route'];
    ...
    $departure_date_formatted = date('Y-m-d', strtotime($start_date));
    $departure_dates_formatted = date('Y-m-d', strtotime($end_date));
}
...
$sql = "SELECT * 
        FROM routes AS r
        INNER JOIN bookings AS b ON r.route_id = b.route_id
        INNER JOIN customers AS c ON b.customer_id = c.customer_id
        WHERE r.route_dep_date BETWEEN '$start_date' AND '$end_date'";
...
echo "</div>";
    ...
    echo "<form method='post' action='http://localhost/SimpleBusTicket-PHP/invoice-db.php'>";
        echo "<input type='hidden' name='customer_route' value='$start_date'>";
        echo "<input type='hidden' name='customer_route' value='$end_date'>";
