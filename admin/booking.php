...
<head>
    ...
    <link rel="stylesheet" href="btnn.css">
        ...
        */
        if($loggedIn && $_SERVER["REQUEST_METHOD"] == "POST")
        {
            ...
            if(isset($_POST["delete"]))
            {
                ...
                $sql = "SELECT *
        FROM bookings
        JOIN routes ON routes.route_id = bookings.route_id
        JOIN customers ON customers.customer_id = bookings.customer_id
        WHERE bookings.route_id = '$route_id'";
                ...
                if ($result->num_rows > 0) {
                    ...
                    $bus=$row["bus_no"];
                    $routee=$row["route_cities"];
                    $seat=$row["booked_seat"];
                    $amount=$row["booked_amount"];
                    $depature=$row["route_dep_date"];




                    $sql = "INSERT INTO book (routee, cust_name, cust_cont, bus, route_no, seat_no, cost_no, dep_no) 
        VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
...
$stmt->bind_param("ssssssss", $pnr, $cname, $cphone, $bus, $routee, $seat, $amount, $depature);
...
$stmt->close();
        ...
        <div class="row">
          <div class="col-lg-12 bg-white shadow p-4 rounded">
                    ...
                    <div class="col2">
          <p class="p"> Filter Buses : </p> 
                 <form method='post' action='http://localhost/SimpleBusTicket-PHP/invoice2.php'>
            <div>
            <select class = "custom-select" name="bus">
                <?php

                //show invoices
                $query=mysqli_query($con,"select distinct bus_no from routes");
                while($invoice=mysqli_fetch_array($query)){
                    echo "<option value='".$invoice['bus_no']."'>".$invoice['bus_no']."</option>";
                }
                ?>
            </select>
            </div>
            <br>
            <input type="submit" class="buttonn" value="Generate"> 
        </form>
            </div>
       
        <div class="col2">
        <p class="p"> Filter Routes : </p> 
                 
                 <form method='post' action='http://localhost/SimpleBusTicket-PHP/admin/routeF.php'>
                     <div>
                     <select class = "custom-select" name="customer_route">
                         <?php
         
                         //show invoices
                         $query=mysqli_query($con,"select DISTINCT customer_route from bookings");
                         while($invoice=mysqli_fetch_array($query)){
                             echo "<option value='".$invoice['customer_route']."'>".$invoice['customer_route']."</option>";
                         }
                         ?>
                     </select>
                     </div>
        ...
        <p class="p"> Filter Dates: </p>

        <form action="http://localhost/SimpleBusTicket-PHP/admin/dateF.php" method="post">
 <div class="col">
        ...
        <button class="buttonn" type="submit">Generate</button>
    </form>
